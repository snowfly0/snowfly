...
public class GenericDataListWindow<T> : DataWindowBase<T> where T : GenericDataList<T>, new()
{
    public class GDataPair
    {
        public string Key;
        public List<T> TValue;
        public GDataPair(string key, List<T> val)
        {
            Key = key;
            TValue = val;
        }
    ...
    protected Dictionary<string, List<T>> _dataMap;
    ...
    void InitLoadData()
    {
        _dataMap = GenericDataList<T>.DataMap;
    ...
    void InitFieldInfos()
    {
        ...
        //List<FieldInfo> fInfos = new List<FieldInfo>(_fielfInfos);
        //for (int i = fInfos.Count - 1; i >= 0;--i )
        //{
        //    if(fInfos[i].FieldType.IsClass)
        //}
    ...
    void DrawValues()
    {
        ...
        for (int i = 0; i < _dataPairs.Count; ++i)
        {
            ...
            pair.Key = EditorGUILayout.TextField(string.Empty, pair.Key, GUILayout.Width(_rowLengths[0]), _heightOfRowOption);
            for (int m = 1; m < _dataPairs[i].TValue.Count; ++m)
            {
                EditorGUILayout.LabelField("", GUILayout.Width(_rowLengths[0]), _heightOfRowOption);
            }
    ...
    void DrawOperations()
    {
        ...
        if (GUILayout.Button("添加新项", GUILayout.Width(80)))
        {
            ...
            newList.Add(newItem);
            var pair = new GDataPair("New", new List<T>(newList));
            _dataPairs.Add(pair);
        }
        ...
        {
            string dir = EditorUtil.GetFileDirectory(GenericDataList<T>.GetEditorFullPath());
            try
            {
    ...
    {
        for (int i = 0; i < pair.TValue.Count; ++i)
        {
            int fieldIndex = 0;
            EditorGUILayout.BeginHorizontal();
            foreach (var info in _fielfInfos)
            {
                ...
                EditorGUIUtil.DrawField(info, pair.TValue[i], string.Empty, option, _heightOfRowOption);
                if (_isFoldouts[fieldIndex] && EditorGUI.EndChangeCheck())
                {
                    object value = info.GetValue(pair.TValue[i]);
                    foreach (var dataPair in _dataPairs)
                    {
                        foreach (var item in dataPair.TValue)
                        {
    ...
    {
        GenericDataList<T>.UnLoadDataMap();
        base.OnDestroy();
    }
}