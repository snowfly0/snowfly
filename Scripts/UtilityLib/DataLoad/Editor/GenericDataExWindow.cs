...
public class GenericDataExWindow<T> : DataWindowBase<T> where T : GenericDataEx<T>, new() 
{
    public class GDataPair
    {
        public string Key;
        public T TValue;
        public GDataPair(string key, T val)
        {
            Key = key;
            TValue = val;
        }
    ...
    protected Dictionary<string, T> _dataMap;
    ...
    void InitLoadData()
    {
        _dataMap = GenericDataEx<T>.DataMap;
    ...
    void DrawOperations()
    {
        ...
        {
            T newItem = new T();
            var pair = new GDataPair("New", newItem);
            _dataPairs.Add(pair);
        }
        ...
        {
            string dir = EditorUtil.GetFileDirectory(GenericDataEx<T>.GetEditorFullPath());
            try
            {
